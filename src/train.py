...
def run():
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=32, collate_fn=(lambda x : collate_fn(x, device)), shuffle=True)
    eval_dataloader = DataLoader(eval_dataset, batch_size=32, collate_fn=(lambda x: collate_fn(x, device)), shuffle=True)
